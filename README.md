...
- **Два режима работы**:
  ...
  - **Локальный режим**: Использует локальную модель YOLO 11s (интернет не требуется)
...
- Обучает модель YOLO 11s на этом датасете
...
**Вариант C: Использовать стандартную модель YOLO 11s**
```bash
...
- **ultralytics** (>=8.0.0): YOLO 11s для локального обнаружения
...
### Локальная модель (YOLO 11s)
- **Архитектура**: YOLO 11s
